...
### Log
- **2026-06-15**: SEO pass for the marketing site. Added `app/lib/site-config.ts` (single source of truth for `SITE_URL`/name/description, public routes, private route prefixes; `SITE_URL` overridable via `NEXT_PUBLIC_SITE_URL`, defaults to `https://readnib.com`). Upgraded `app/layout.tsx` root `metadata` with `metadataBase`, title template, keywords, canonical, Open Graph + Twitter cards, and a `robots` policy, and injected Organization/WebSite/SoftwareApplication JSON-LD in `<head>`. Added `app/sitemap.ts` (per-route changefreq/priority over the public routes), `app/robots.ts` (allows public, disallows app/api routes, points at the sitemap), and `app/opengraph-image.tsx` (dynamic 1200×630 branded share card via `next/og`). Verified all three routes (`/sitemap.xml`, `/robots.txt`, `/opengraph-image`) emit statically in `next build`.
