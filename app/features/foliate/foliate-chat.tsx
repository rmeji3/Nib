...
import { Icon } from './foliate-viewer';
import type {
  AssistantMessage,
  Citation,
  MessageSegment,
  ...
  return (
    <div className="suggestions flex flex-col gap-2">
      ...
      {PROMPT_LIBRARY.map((prompt) => (
        <button key={prompt.q} type="button" className="suggestion flex items-center gap-2.5 rounded-lg border border-white/10 px-3 py-2.5 text-left text-[13px] text-[var(--text)] transition hover:bg-[var(--bg-elevated)]" onClick={() => onPick(prompt)}>
          <span className="suggestion-icon inline-flex h-[22px] w-[22px] items-center justify-center rounded-md bg-[var(--bg-elevated)] text-[var(--text-dim)]"><Icon name={prompt.icon} /></span>
          <span>{prompt.q}</span>
        </button>
      ))}
    </div>
...
function Composer({ onSend, disabled }: { onSend: (value: string) => void; disabled: boolean }) {
  ...
  return (
    <div className="border-t border-white/10 bg-[linear-gradient(180deg,rgba(17,20,26,0.7)_0%,var(--bg-surface)_60%)] px-3.5 pb-3.5 pt-3">
      <div className="composer-inner relative rounded-xl border border-white/15 bg-[var(--bg-base)]">
        <textarea
          ref={textareaRef}
          rows={1}
...
export function ChatPanel({
  ...
  return (
    <div className={`chat relative grid min-h-0 grid-rows-[auto_1fr_auto] overflow-hidden border-l border-white/10 bg-[var(--bg-surface)]${busy ? ' is-thinking' : ''}`}>
          ...
          <div className="chat-empty flex flex-col gap-4">
            <div className="chat-welcome pb-3">
              ...
              <p className="text-[12.5px] leading-[1.45] text-[var(--text-dim)]">
                Foliate has indexed every paragraph, the table on page 4, and both figures. Every answer is grounded with page-level citations you can click to jump to.
              </p>
            </div>
